...
print("Found " + str(len(res_videos)) + " videos in Spring 2022")
...
for i in range(0, len(res_videos)):
    transcribeVideo(videos_folder, res_videos[i], i, len(res_videos), model, "Spring 2022", "forward")


###### Uncomment to run from the last file to the first file ######   
 
# for i in range(len(res_videos) - 1, -1, -1):
    # transcribeVideo(videos_folder, res_videos[i], i, len(res_videos), model, "Spring 2022", "backward")

##### Uncomment to run the processing after finishing #####

#subprocess.run(["python", "Processing_RQ3_noGUI_FINAL.py"])