...
class MainWindow(QWidget):
    ...
    global audioFps, audioFrames, fileFound, filename, vidFrames, vidLength, Init_Frame, slider, img_array, processFin, img_size
    ...
    def __init__(self):
        ...
        global audioLength, audioFps, audioFrames, fileFound, filename, vidFrames, vidLength, Init_Frame, slider, img_array
        ...
        self.setWindowTitle("N-GAGE (Processing)")
    ...
    def open_file(self):
        ...
        global audioSource, audioLength, audioFps, audioFrames, fileFound, filename, vidFrames, vidLength, img_array, processFin
...
class Worker1(QThread):
    ...
    global fileFound, filename, Capture, CurrFrame, vidFrames, vidLength, Init_Frame, slider, fps, sleep_ms, img_array, processFin
    ...
    def run(self):
        ...
        global sleep_ms, img_array, processFin, img_size, PauseBTN, PlayBTN, openBtn, out_filename, mediaPlayer, videowidget, progress_label
        ...
        while self.ThreadActive:
            if int(Capture.get(cv2.CAP_PROP_POS_FRAMES)) >= vidFrames:
                ThreadActive = self.stop()
                self.close(Capture)
                self.write_video(filename, all_data_out, fps, img_size, img_array)
                break
            ...
            if text_x_0 != dead_x:
                w_frame = cv2.circle(w_frame, (text_x_0, text_y_0), 20, text_point_color, -1)
            ...
            if audio_x_0 != (dead_x):
                w_frame = cv2.circle(w_frame, (audio_x_0, audio_y_0), 20, audio_point_color, -1)
            ...
            if video_x != dead_x:
                if audio_x_0 != (dead_x):
                    w_frame = cv2.line(w_frame, (audio_x_0, audio_y_0), (video_x, video_y), (0,0,0), 2)
                    
                if text_x_0 != dead_x:
                    w_frame = cv2.line(w_frame, (text_x_0, text_y_0), (video_x, video_y), (0,0,0), 2)
                    
                if audio_x_0 != (dead_x) and text_x_0 != dead_x:
                    w_frame = cv2.line(w_frame, (text_x_0, text_y_0), (audio_x_0, audio_y_0), (0,0,0), 2)
    ...
    def begin(self):
        global audioSource, audioLength, audioFps, audioFrames, fileFound, filename, Capture, CurrFrame, img_array
        global processFin, img_size, PauseBTN, PlayBTN, openBtn, out_filename, mediaPlayer, videowidget, progress_label
        self.ThreadActive = True
        print("Play video")
        print(CurrFrame)
    ...
    def onChange(self, trackbarValue):
        ...
        global slider, img_array, processFin, img_size, PauseBTN, PlayBTN, openBtn, out_filename, mediaPlayer, videowidget, progress_label
    ...
    def write_video(self):
        global all_data_out, text_data_out, video_data_out, audioSource, audioLength, audioFps, audioFrames, img_array
        ...
        # np.savetxt(out_filename.replace('.mp4','') + "_video_data.csv", video_data_out, delimiter=",")
        # np.savetxt(out_filename.replace('.mp4','') + "_text_data.csv", text_data_out, delimiter=",")
        
        #video_data_out.to_csv(out_filename.replace('.mp4','') + "_video_data.csv", index=False)
        #text_data_out.to_csv(out_filename.replace('.mp4','') + "_text_data.csv", index=False)
        all_data_out.to_csv(out_filename.replace('.mp4','') + "_all_data.csv", index=False)
        #out = cv2.VideoWriter('vids/project.avi',cv2.VideoWriter_fourcc(*'DIVX'), 15, img_size)
