...
def write_video(filename, all_data_out, fps, img_size, img_array):
    ...
    #MixAudioVideo(out_filename, filename.replace('.mp4','.wav'), filename.replace('.mp4','_processed_wSound.mp4'))
    
    time.sleep(1)
    
    #del all_data_out, out, img_array
...
if __name__ == "__main__":
    ...
    ### Processes from the last file to the first ###
    if len(res) != 0:
        for i in range(len(res) - 1, -1, -1):
            processFile(res[i],i,len(res))
